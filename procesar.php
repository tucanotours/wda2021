...
if (isset($_POST['encuestapopup'])) {
	...
	$_POST['popuprating4'] = limpiar_ingreso($_POST['popuprating4']);
		...
		*/
		...
		$url = "Location: ".$conf_root."index.php?encuesta=".$agregado;
		header($url);
...
if (isset($_POST['nuevo_registro'])) {
	...
	*/
	...
	if ($encontroemail==1) {
		$url = "Location: ".$conf_root."index.php?emailyaexiste=".$_POST['email'].$agregado;
		header($url);
	} else {
	
		// validaciones
		if(!preg_match("/^[[:alnum:]][a-z0-9_.-]*@[a-z0-9.-]+\.[a-z]{2,4}$/i", $_POST['email'])) { // valido email
			$url = "Location: ".$conf_root."index.php?email_incorrecto=".$agregado;
			header($url);
		}
	...
	<!--/100% background wrapper-->
		...
		$estadoEnvio2 = $mail->Send();
			...
			if($estadoEnvio){
				...
				$url = "Location: ".$conf_root."index.php?cuenta=".$email."&nombre=".$nombre.$agregado;
				header($url);
			
			} else {
				
				$url = "Location: ".$conf_root."index.php?error-email".$agregado;
				header($url);
